...
export async function transcribeEpisodeHandler(payload: TranscribeEpisodePayload) {
  ...
  const ext = originalFilename.includes('.') ? originalFilename.split('.').pop() : 'mp3';

  let text, language, segments;

  try {
    if (fileSizeInBytes > MAX_FILE_SIZE) {
        console.log(`[Transcription] File size (${(fileSizeInBytes / 1024 / 1024).toFixed(2)}MB) exceeds 25MB. Uploading to R2...`);
        ...
        try {
            const publicUrl = await uploadFileToStorage(fileBuffer, storageKey, contentType);
        ...
        console.log(`[Transcription] File size is small. Calling Groq directly...`);
        // Read file buffer
        const fileBuffer = fs.readFileSync(tempFilePath);
