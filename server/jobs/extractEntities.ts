...
import { episodes, transcripts, entities, episodesEntities, entityTypes } from '../database/schema';
  ...
  try {
    ...
    for (const entity of extracted) {
        ...
        if (existingType) {
            ...
            try {
                const [insertedType] = await db.insert(entityTypes).values({
                    name: normalizedType
                ...
                if (retryType) {
                    ...
                    console.error(`[Entities] Failed to insert type ${normalizedType}`, e);
                    continue;
                }
            }
        }
            ...
            try {
               ...
               entityId = inserted.id;
                ...
                if (retry) {
                    ...
                    console.error(`[Entities] Failed to insert entity ${entity.name}`, e);
                    continue;
                }
            }
        }
        ...
        // Link to Episode
        await db.insert(episodesEntities).values({
            episodeId,
            entityId
        }).onConflictDoNothing();
    }
