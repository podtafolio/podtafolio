...
describe('groq utils', () => {
  ...
  it('should throw error if API key is missing', async () => {
    ...
    await expect(transcribeAudio(buffer, 'test.mp3')).rejects.toThrow('GROQ_API_KEY environment variable is not set')
  })
  ...
  it('should throw error if API returns error', async () => {
    ...
    const buffer = Buffer.from('test')
    await expect(transcribeAudio(buffer, 'test.mp3')).rejects.toThrow('Groq API failed: 400 Bad Request - Invalid file')
  })
})
