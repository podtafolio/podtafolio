...
describe('feedParser utils', () => {
  ...
  it('should throw error if feed is missing title', async () => {
    ...
    await expect(parsePodcastFeed('https://example.com/rss')).rejects.toThrow('Failed to parse podcast feed: Feed is missing a title.')
  })
  ...
  it('should parse duration correctly', async () => {
     ...
     const mockFeed = {
      title: 'Duration Test',
      items: [
        { title: 'Secs', enclosure: { url: 'a' }, guid: '1', itunesDuration: '3600' }, // 3600s
        { title: 'MM:SS', enclosure: { url: 'b' }, guid: '2', itunesDuration: '10:00' }, // 600s
        { title: 'HH:MM:SS', enclosure: { url: 'c' }, guid: '3', itunesDuration: '01:00:00' }, // 3600s
        { title: 'Number', enclosure: { url: 'd' }, guid: '4', duration: 120 }, // 120s (raw number)
        { title: 'Invalid', enclosure: { url: 'e' }, guid: '5', itunesDuration: 'invalid' }, // undefined
      ]
    }
