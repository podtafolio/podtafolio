...
describe("cache utils", () => {
  ...
  describe("invalidatePodcastCache", () => {
    it("should clear group caches and specific podcast item", async () => {
      ...
      // Verify group clears
      expect(mocks.storage.clear).toHaveBeenCalledWith(
        groupPrefix + CACHE_NAMES.PODCASTS_LIST,
      );
      expect(mocks.storage.clear).toHaveBeenCalledWith(
        groupPrefix + CACHE_NAMES.EPISODES_LIST,
      );
      expect(mocks.storage.clear).toHaveBeenCalledWith(
        groupPrefix + CACHE_NAMES.PODCAST_EPISODES,
      );
      expect(mocks.storage.clear).toHaveBeenCalledWith(
        groupPrefix + CACHE_NAMES.SEARCH,
      );
